...
from ai_core.data_source.utils import create_collection_name, create_data_source_id, split_texts
...
async def create_datasource(
    ...
    limit: Optional[int] = Form(None, description="Limit for JIRA issues"),
    ...
    # start datasource_type
    # pydantic_model = models.Datasource.from_orm(db_datasource)
...
async def preview_datasource(
    ...
    if existing_datasource:
        ...
        info_datasource = models.Datasource(
            ...
            datasource_type=datasource_type.value,  # e.g., set a default type
    ...
    if datasource_type == DataSourceType.CONFLUENCE:
        ...
        preview_data = data_source.load_preview_data(url=url, access_token=session_data.token_confluence, space_key=space_key)
    
    if datasource_type == DataSourceType.JIRA:
        if utils.is_empty(session_data.token_jira):
            raise HTTPException(status_code=422, detail=f"Token of Jira is required for {datasource_type.name} datasource type")
        preview_data = data_source.load_preview_data(url=url, access_token=session_data.token_jira, project_key=project_key, start=start)
    ...
    if datasource_type == DataSourceType.URL:
        preview_data = data_source.load_preview_data(url=url, base_url=base_url)
...
async def create_embedding(
    ...
    # 4. 데이터를 텍스트 파일로 저장
    data_size = 0
    if data_source_type == DataSourceType.TEXT:
    ...
    if data_source_type == DataSourceType.PDF_FILE:
    ...
    if data_source_type == DataSourceType.DOC_FILE:
    ...
    if data_source_type == DataSourceType.CONFLUENCE:
        if utils.is_empty(session_data.token_confluence):
            raise HTTPException(status_code=422, detail=f"Token of Confluence is required for {data_source_type.name} datasource type")
    ...
    if data_source_type == DataSourceType.JIRA:
        if utils.is_empty(session_data.token_jira):
            raise HTTPException(status_code=422, detail=f"Token of Jira is required for {data_source_type.name} datasource type")
        # if os.path.exists(data_source.data_dir_path + '/0'):
        #     data_size = get_total_file_size(data_source.data_dir_path + '/0')
        save_task = asyncio.create_task(
            data_source.save_data(
                url=db_datasource.url ,
                access_token=session_data.token_jira,
    ...
    if data_source_type == DataSourceType.GITLAB:
        if utils.is_empty(session_data.token_gitlab):
            raise HTTPException(status_code=422, detail=f"Token of Gitlab is required for {data_source_type.name} datasource type")
        # if os.path.exists(data_source.data_dir_path + '/0'):
        #     data_size = get_total_file_size(data_source.data_dir_path + '/0')
    ...
    if data_source_type == DataSourceType.GITLAB_DISCUSSION:
        if utils.is_empty(session_data.token_gitlab):
            raise HTTPException(status_code=422, detail=f"Token of Gitlab is required for {data_source_type.name} datasource type")
        # if os.path.exists(data_source.data_dir_path + '/0'):
        #     data_size = get_total_file_size(data_source.data_dir_path + '/0')
    ...
    logging.info("Data saving task started")
    ...
    if 'splitter' in splitter_payload_data :
    ...
    def embed_callback(future):
        if future.exception():
            ...
            logging.info("Update embedding state to failed")
        else:
            embeded, total = future.result()
            logging.info("Embedding task completed. Number of chunks embedded / Total: ", str(embeded), " / ", str(total))
